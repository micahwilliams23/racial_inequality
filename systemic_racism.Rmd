...
county_income <- get_acs('county',
                         year = 2018,
                         variables = c('B19013B_001', 'B19013A_001'),
                         cache_table = T,
                         geometry = T) %>%
  mutate(race = case_when(variable == 'B19013B_001' ~ 'Black',
...
county_income %>%
  group_by(GEOID) %>%
  nest(data = c(NAME, variable, estimate, moe, race)) %>%
  ungroup() %>%
  slice(1) %>%
  .$data
  summarize(income.diff = map_dbl(.[[1]]$estimate, ~.[1]-.[2]))
  
  # ungroup() %>%
  # slice(1)
  summarize(income.diff = map_dbl(data[[1]]$estimate, ~.[[1]][1]))
  full_join(black_fips, by = c('GEOID'))

# create linear model of income and percet black
percent_income_model <- lm(income ~ percent, data = county_income)
rsq <- summary(percent_income_model)$r.squared
```


```{r plotly}
# draw plot of percent black vs median household income
county_plot <- county_income %>%
  ggplot(aes(text = paste(NAME,
                          '\nBlack Population: ', prettyNum(black.pop, big.mark = ','),
                          ' (', round(percent,1), '%)',
                          '\nMedian Income: $', prettyNum(income, big.mark = ','), sep = ''))) +
  
  # add points for counties and line of best fit
  geom_sf(aes(percent, income),
  alpha = 0.2) +
  geom_smooth(aes(percent, income),
              alpha = 1, color= 'red', size = 5) +
  # geom_abline(aes(intercept = percent_income_model$coefficients['(Intercept)'], 
  #                 slope = percent_income_model$coefficients['percent']))
  
  labs(title = 'Median Black Household Income vs. Percent Black Population, by County',
       subtitle = 'Includes data for 100 counties with highest percent black population.',
       y = 'Median Household Income',
       x = 'Percent Black') +
  
  scale_x_continuous(breaks = seq(0,90,15),
                     labels = paste(seq(0,90,15),'%', sep = '')) +
  scale_y_continuous(breaks = seq(0,200000,40000),
                     labels = paste('$',seq(0,200,40),'K', sep = '')); county_plot
