...
prison.race <- prison.18 %>%
  ...
  # add data for national total
  full_join(bjs_total %>%
              filter(year >= 2008) %>%
              mutate(race = 'national rate'), 
            by = c('year', 'race', 'rate'))
...
race.labels <- race.dots %>% mutate(label.y = case_when(
  year == 2018 & race == 'black' | race == 'national rate' ~ rate + 170,
  TRUE ~ rate - 170))

# import csv from BJS Corrections Statistics Analysis site (national rate per 100,000)
bjs_total <- read_csv('data/prison/bjs_totals.csv', skip = 9) %>% 
  slice(1) %>%
  select(-c(Jurisdiction, X2, X42)) %>% 
  pivot_longer(cols = c(`1978/1`:`2016`),
               names_to = 'year',
               values_to = 'rate') %>%
  mutate(year = as.numeric(str_remove(year, '(/[:digit:]+)+')))

# create year labels for plot
year_labels <- paste('\'', str_remove(seq(1980,2015,5), '[:digit:]{2}'), sep = '')
year_labels[5] <- '2000'

# create tibble of dots to highlight and labels
bjs.dots <- bjs_total %>% filter(year %in% c(1978, 2007, 2016))
bjs.labels <- dots %>% mutate(label.y = case_when(year == 1978 ~ rate + 35,
                                              TRUE ~ rate - 35))
...
prison.race %>%
  ...
  scale_color_manual(values = c('#377EB8', '#003333', '#6baed6')) +
...
ggplot(aes(year, rate), data = bjs_total) +
  ...
  labs(title = 'United States Incarceration Rate, 1978-2016',
       subtitle = 'The incarceration rate in the US nearly quadrupled from 1978 to 2007, but the rate has been in decline since the\nlate 2000\'s.',
       x = 'Year', y = 'Incarceration Rate\n(per 100,000)',
       caption = 'Source: Bureau of Justice Statistics') +
